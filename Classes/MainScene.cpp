...
void MainScene::goNextLevel()
{
	unscheduleUpdate();
	level ++;
	ShareData::getInstance()->rankScore += ( level*100 + ShareData::getInstance()->heroScore / int(nowTime+1) )* hard*hard;
...
void MainScene::addPhysics()
{
	...
	for (auto& obj : objectGroupBall) //添加跷跷板
	{
